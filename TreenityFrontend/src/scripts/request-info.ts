...
document.addEventListener('DOMContentLoaded', () => {
  ...
  form.addEventListener('submit', async (e) => {
    ...
    if (!/\S+@\S+\.\S+/.test(data.email as string)) errors.push('Email non valida');
    if (!/^\+39[0-9]{9,10}$/.test(data.phone as string)) errors.push('Numero di telefono non valido');
    if (!data.groupType) errors.push('Seleziona il tipo di gruppo');
    if (!data.message) errors.push('Il messaggio è obbligatorio');
    if (!data.consentForm) errors.push('Devi accettare i termini e le condizioni');
